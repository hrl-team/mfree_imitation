function [ out ] = MB4( params,pnames,learning_data,post_learning_data,simulation,prior)
    ...
    for i = 1:length(learning_data(:,TRIAL))
        ...
        if (~isnan(learning_data(i,AGENT)) && ~isnan(learning_data(i,CHOICE)))
            ...
            if(learning_data(i,CONDITION)~=0)
                ...
                sAPE = 1-P(learning_data(i,STATE),dem);

                %P(learning_data(i,STATE),dem) = max(0,P(learning_data(i,STATE),dem) + alpha_sape * sAPE);
                P(learning_data(i,STATE),dem) = P(learning_data(i,STATE),dem) + alpha_sape * sAPE;
                P(learning_data(i,STATE),3-dem) = 1-P(learning_data(i,STATE),dem);
            end
